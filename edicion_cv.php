...
if ($connect) {
		...
		$consulta="UPDATE `sirccami_bd`.`aspirante` SET `cedula`='$cedula', `nombre`='$nombre', `apellido`='$apellido', `fecha_nac`='$fecha_nac', `estado_civ`='$estado_civ', `sexo`='$sexo', `nacionalidad`='$nacionalidad', `telefono`='$telefono', `email`='$email', `nivel_acdm`='$nivel_acdm', `instruccion_usr`='$instruccion_usr', `cursos`='$cursos', `certf`='$certf', `emp_ant`='$emp_ant', `nombre_emp_ant`='$nombre_emp_ant', `cargo_ant`='$cargo_ant', `nombre_ref_1`='$nombre_ref_1', `telf_ref_1`='$telf_ref_1', `nombre_ref_2`='$nombre_ref_2', `telf_ref_2`='$telf_ref_2' WHERE `id`='$idN';";
		
		$resultado=mysqli_query($connect,$consulta);
		
		if ($resultado) {
			echo "perfil actualizado. <br />";
		}
		else {
	...
	if (in_array($_FILES['files']['type']['0'], $permitidos) && $_FILES['files']['size']['0'] <= $limite_kb * 1024){
		...
		//o pueden darle otro nombre para que no sobreescriba el actual.
			...
			if ($resultado){
				...
				$consulta_2="UPDATE `sirccami_bd`.`aspirante` SET `foto`='$nombre_foto' WHERE `id`='$idN';" ;
